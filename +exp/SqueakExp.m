...
%     Audio = aud.AudioRegistry
  ...
  methods
    function obj = SqueakExp(paramStruct)
      ...
      obj.Audio = naud.AudioRegistry(96e3);
      ...
      paramStruct.defFunction(...
        obj.Time, obj.Events, obj.Params, obj.Visual, obj.Inputs , obj.Outputs, obj.Audio);
      % listeners
      obj.Listeners = [
        obj.Events.expStart.into(advanceTrial) %expStart signals advance
      ...
      %% data struct
      obj.Data.expDef = func2str(paramStruct.defFunction);
%       obj.Params = obj.Params.map(@(v)v, [], @(n,s)sig.Logger([n '[L]'],s));
    ...
    function data = run(obj, ref)
      ...
      start.addCallback(@(varargin) obj.Events.expStart.post(true));
